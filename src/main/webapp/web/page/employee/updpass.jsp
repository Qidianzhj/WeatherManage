...
<form class="layui-form layui-form-pane" method="post" lay-filter="form">
  ...
  <div class="layui-form-item">
    ...
    <input type="button"class="layui-btn" lay-submit lay-filter="updpass" value="确认修改">
  </div>
</form>
</div>
 </fieldset>
...
function init(){
	...
	$.ajax({
	    ...
	    success:function(d){
	    	console.log(d);
	    	form.val("form",{sno:d.sno,pass:d.pass});
	    }
	});
}
init();
form.on("submit(updpass)",function(data){//layui的时间触发方式
	$.ajax({
		url:'/demo1/employee/updatepass',
		type:'post',
